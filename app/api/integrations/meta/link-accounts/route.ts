...
export async function POST(request: Request) {
  ...
  const linkedAccounts = await Promise.all(accounts.map((account) => prisma.linkedAdAccount.upsert({
    where: { workspaceId_platform_externalAccountId: { workspaceId, platform: 'META', externalAccountId: account.externalAccountId } },
    update: { accountName: account.accountName, currency: account.currency, timezone: account.timezone, status: 'ACTIVE' },
    create: { workspaceId, connectionId: connection.id, rootEntityId, platform: 'META', externalAccountId: account.externalAccountId, accountName: account.accountName, currency: account.currency, timezone: account.timezone, status: 'ACTIVE' }
  })));
